  ...
  cy.session([email, password], () => {
    ...
    cy.request({
      ...
      window.localStorage.setItem(
        "questionable-portal.demo.demo-auth",
        `{"accessToken":"${tokenByAPI}"}`
      );
  ...
  cy.session([email, password], () => {
    ...
    cy.request({
      ...
      cy.request({
        ...
        window.localStorage.setItem(
          "questionable-portal.demo.sessionId",
          `"${fe_session_id}"`
        );
...
Cypress.Commands.add("clickAll", (elmCSS) => {
  cy.get(elmCSS).as("checkboxes").check();
});
Cypress.Commands.add("clickAllXpath", (elmXpath) => {
  //not work till 15 Nov
  cy.xpath(elmXpath).as("checkboxes").check();
});
...
Cypress.Commands.add("loginByPredefinedAccessToken", (email, password) => {
  ...
  cy.request({
    ...
    window.localStorage.setItem(
      "gotit.mathgpt-educator.authenticated_access_token",
...
Cypress.Commands.add("loginByAccessToken", (url, email, code, accessToken) => {
  ...
  cy.request({
    ...
    window.localStorage.setItem(
      "gotit.mathgpt.authenticated_access_token",
      accessToken
    );
...
Cypress.Commands.add(
  "loginByAPIgetRefreshToken",
  (token_refreshes_api_url, original_refresh_token) => {
    // cy.session([email, password], () => {
    //step 1: send request to get token
    cy.request({
      ...
      window.localStorage.setItem(
        "gotit.mathgpt.authenticated_refresh_token",
        refresh_token
      );
...
Cypress.Commands.add(
  "loginAPIThenGetAccessTokenAPI",
  (login_api_get_refresh_token, token_refreshes_api_url, email, password) => {
    // Step 1: Login API
    cy.request({
      ...
      cy.request({
        ...
        let data = [];
        data.push({ access_token: access_token, refresh_token: refresh_token });
        cy.writeFile(filename, data);
        // cy.writeFile(filename1, {
        //   'access_token': access_token,
        //   'refresh_token': refresh_token
